...
const [userUpdateState, setUsertUpdateState] = useState<Object>();
    ...
    const handleShowUserProfile = async () => {
        await dispatch(
             fetchUserProfile(user.userEmailId)
           )
...
{detailUser.avatar && (
                            <img src={detailUser.avatar} style={{ width: '100px', height: '100px', borderRadius: '50%', marginRight: '24px' }} alt='avatar' />
                        )}
                                <div>

                                    <Typography className='fw-bold' variant="h5">{detailUser.username}</Typography>
                                </div>
